"""
完整流程：创建→begin→提交所有题答案→finish→detail（含答案解析）
"""
import json
...
print(f'2. begin: testId={new_tid}')
...
questions = r.json()['data']['paperQuestionDetails']
print(f'3. 获取 {len(questions)} 题')
...
for q in questions:
    if q['chooseAnswer']:
        ...
        r = requests.post('https://gw-c.nowcoder.com/api/sparta/test/recordAnswer',
                          json={'questionId': q['id'], 'paperId': pid, 'testId': new_tid, 'answerContent': ans},
                          headers=HEADERS, impersonate='chrome124', timeout=10)
        print(f'   提交题{q["index"]}: {r.json().get("code")}')
...
print(f'4. finish: {r.json()}')

# 6. 交卷后获取 detail（关键！看是否有答案解析）
r = requests.post('https://gw-c.nowcoder.com/api/sparta/test/detail', json={'testId': new_tid, 'paperId': pid}, headers=HEADERS, impersonate='chrome124', timeout=15)
detail = r.json()['data']
questions_after = detail['paperQuestionDetails']
print(f'\n5. 交卷后 detail: {len(questions_after)} 题')

q0 = questions_after[0]
print(f'\n题1 字段: {list(q0.keys())}')
# 检查所有可能的答案字段
for key in ['analysis', 'referenceAnswer', 'rightAnswer', 'answer', 'userAnswerContent', 'questionStatus', 'correctAnswer', 'answered']:
    if key in q0 and q0[key]:
        print(f'\n  ★ {key}: {str(q0[key])[:500]}')

# 检查选项是否有 correct/right
if q0.get('chooseAnswer'):
    print(f'\n  选项字段: {list(q0["chooseAnswer"][0].keys())}')
    for o in q0['chooseAnswer']:
        print(f'    {o}')

# 完整打印第一题
print(f'\n=== 题1 完整（交卷后）===')
print(json.dumps(q0, ensure_ascii=False, indent=2)[:2000])
